...
function updateConnectivityStatus(online) {
    ...
    if (!online) {
        banner.textContent = 'No internet connection. Please check your Wi-Fi or mobile data.';
...
document.getElementById('signupForm')?.addEventListener('submit', async function(e) {
    ...
    if (name.length < 2) { showModalAlert('signupAlert', 'Please enter your full name (at least 2 characters).'); return; }
    ...
    if (!terms) { showModalAlert('signupAlert', 'Please accept the terms and conditions.'); return; }
    ...
    try {
        ...
        if (data.success) {
            ...
            showModalAlert('signupAlert', data.message || 'Sign-up failed.');
            btn.textContent = orig;
            btn.disabled = false;
        }
    } catch (err) {
...
document.getElementById('signup-password')?.addEventListener('input', function() {
    ...
    else if (len < 6) { help.textContent = 'Too short. Use at least 6 characters.'; help.style.color = '#dc2626'; }
    else if (len < 9) { help.textContent = 'Use a longer password for better security.'; help.style.color = '#d97706'; }
